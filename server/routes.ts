...
export async function registerRoutes(app: Express): Promise<Server> {
      ...
      try {
        ...
        if (
          ...
          if (match) {
            ...
            const recipientConn = connections.find(
              (conn) => conn.userId === recipientId,
            );
            if (
              recipientConn &&
              recipientConn.socket.readyState === WebSocket.OPEN
            ) {
              recipientConn.socket.send(
                JSON.stringify({
                  type: "new_message",
                  message: newMessage,
                }),
        ...
        if (
          data.type === "video_signal" &&
          userId &&
          data.targetUserId &&
          data.signal
        ) {
          const targetConn = connections.find(
            (conn) => conn.userId === data.targetUserId,
          );
  ...
  app.patch("/api/users/:id", async (req, res) => {
    if (
      !req.isAuthenticated() ||
      (req.user as any).id !== parseInt(req.params.id)
    ) {
  ...
  app.post("/api/matches", async (req, res) => {
    ...
    try {
      ...
      res
        .status(201)
        .json({ ...match, compatibilityReasons: compatibility.reasons });
    } catch (error) {
      if (error instanceof z.ZodError) {
        return res
          .status(400)
          .json({ message: "Invalid match data", errors: error.errors });
      }
  ...
  app.get("/api/matches/:matchId/conversation-starters", async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ message: "Error generating conversation starters" });
    }
  });
    ...
    try {
      ...
      if (match.userId1 !== userId && match.userId2 !== userId) {
        return res
          .status(401)
          .json({
            message: "Not authorized to view video calls for this match",
          });
      }
    ...
    try {
      ...
      if (match.userId1 !== userId && match.userId2 !== userId) {
        return res
          .status(401)
          .json({
            message: "Not authorized to schedule video calls for this match",
          });
      }
      ...
      if (error instanceof z.ZodError) {
        return res
          .status(400)
          .json({ message: "Invalid video call data", errors: error.errors });
      }
  ...
  app.patch("/api/video-calls/:id", async (req, res) => {
    ...
    try {
      ...
      if (match.userId1 !== userId && match.userId2 !== userId) {
        return res
          .status(401)
          .json({ message: "Not authorized to update this video call" });
      }
      ...
      const updatedVideoCall = await storage.updateVideoCallStatus(
        videoCallId,
        status,
        duration,
      );
