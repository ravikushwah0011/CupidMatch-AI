...
export default function ProfileCard({ profile, onLike, onPass }: ProfileCardProps) {
  ...
  return (
    <div className="profile-card bg-white rounded-xl shadow-card mb-6 overflow-hidden">
        ...
        <div className="video-placeholder relative">
          ...
          {profile.profileVideoUrl ? (
            ...
            <div className="absolute inset-0 bg-neutral-800 flex items-center justify-center">
              <span className="text-white text-opacity-50">No video available</span>
            </div>
          )}
          ...
          <div className="absolute bottom-4 left-4 right-4 text-white">
            <div className="flex justify-between items-end">
              <div>
                <h3 className="text-2xl font-semibold">{profile.profileName}, {profile.age}</h3>
      ...
      <div className="p-4">
        <div className="mb-3">
          <h4 className="text-lg font-semibold mb-1">About {profile.profileName}</h4>
          <p className="text-neutral-700 text-sm">{profile.bio}</p>
        </div>
        
        <div className="mb-3">
          <h4 className="text-sm font-semibold text-neutral-600 mb-2">Interests</h4>
          <div className="flex flex-wrap gap-2">
            {profile.interests.map((interest, index) => (
              <span key={index} className="bg-neutral-100 px-3 py-1 rounded-full text-xs">{interest}</span>
            ))}
          </div>
        </div>
