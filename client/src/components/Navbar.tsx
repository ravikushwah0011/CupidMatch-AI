...
export default function Navbar() {
  ...
  const isActive = (path: string) => {
    ...
    if (path === '/messages' && location.startsWith('/messages') || location.startsWith('/chat/')) return true;
    if (path === '/profile' && location === '/profile') return true;
    return false;
  };

  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-neutral-200 py-3 px-6 z-10 max-w-md mx-auto">
